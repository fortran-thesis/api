...
describe("auth lib (integration)", () => {
  ...
  describe("getAuthUserBy (id or email)", () => {
    it(
      "should retrieve user with its details when getting auth user by id",
      async () => {
        const result = await auth.getAuthUserById(testUid);
        expect(
          result && result.id && result.user && result.details
        ).toBeDefined();
    ...
    it(
      "should return null if id of auth user is not found",
      async () => {
    ...
    it(
      "should retrieve user with its details when getting auth user by email",
      async () => {
    ...
    it(
      "should return null if email of auth user is not found",
      async () => {
  ...
  describe("verify (cookie or token)", () => {
    it(
      "should return user when verifying token",
      async () => {
        jest
          .spyOn(getAuth(), "verifyIdToken")
          .mockImplementation(async () => ({uid: testAPIUser.id}) as any);
    ...
    it(
      "should return user when verifying cookie",
      async () => {
        jest
          .spyOn(getAuth(), "verifySessionCookie")
          .mockImplementation(async () => ({uid: testAPIUser.id}) as any);
