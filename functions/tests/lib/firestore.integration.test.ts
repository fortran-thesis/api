...
describe("firestore lib (integration)", () => {
  describe("handle CRUD operations", () => {
    ...
    it("should not add duplicates.", async () => {
      ...
      await expect(
        addDocument(TEST_COLLECTION, testData, "test_id")
      ).resolves.toBeNull();
    });
    ...
    describe("handle retrieval of data", () => {
      ...
      it("should get document ID by field", async () => {
        ...
        const addRes = await addDocument(TEST_COLLECTION, testData);
        const docId = await getDocumentIdByField(
          TEST_COLLECTION,
          "name",
          "idbyfielduser"
        );
      ...
      it("should get document snapshot by field", async () => {
        ...
        await addDocument(TEST_COLLECTION, testData);
        const docSnap = await getDocumentByFieldId(
          TEST_COLLECTION,
          "name",
          "snapbyfielduser"
        );
      ...
      describe("pagination", () => {
        ...
        it("should retrieve first page of paginated documents", async () => {
          const result = await getPaginatedDocuments(
            PAGINATE_COLLECTION,
            PAGE_SIZE,
            undefined,
            ["value"]
          ...
          expect(names).toEqual(
            testRecords.slice(0, PAGE_SIZE).map((r) => r.name)
          );
        ...
        it("should retrieve second page of paginated documents", async () => {
          ...
          const names = result!.snapshot.docs.map((doc) => doc.data().name);
          expect(names).toEqual(
            testRecords.slice(PAGE_SIZE, PAGE_SIZE * 2).map((r) => r.name)
          );
        });
    ...
    it("should not update a non-existing user", async () => {
      await expect(
        updateDocument(TEST_COLLECTION, "non-existed document", {value: 2})
      ).resolves.toBeNull();
    });
    ...
    it("should not delete a non-existing user", async () => {
      await expect(
        deleteDocument(TEST_COLLECTION, "non-existed document")
      ).resolves.toBeNull();
    });
  ...
  it("should not soft delete a non-existing user", async () => {
    await expect(
      softDeleteDocument(TEST_COLLECTION, "non-existed document")
    ).resolves.toBeNull();
  });
