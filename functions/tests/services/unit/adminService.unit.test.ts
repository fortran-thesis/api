...
describe('adminService (unit)', () => {
  ...
  describe('banUser', () => {
    it('should successfully ban a user', async () => {
      ...
      mockUserRepository.findFirestoreUserById.mockResolvedValue(mockUserDoc as any);
      ...
      expect(mockUserRepository.updateFirestoreUser).toHaveBeenCalledWith(userId, { is_banned: true });
      expect(mockEmailUtils.sendEmail).toHaveBeenCalledWith(
        email,
