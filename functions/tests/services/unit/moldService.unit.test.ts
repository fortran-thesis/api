...
describe('moldService (unit)', () => {
  ...
  describe('addMoldToFirestore', () => {
    it('should successfully add a mold', async () => {
      ...
      expect(mockRedisUtils.deleteCachePattern).toHaveBeenCalledWith('molds:list:*');
    });
  ...
  describe('retrieveMoldById', () => {
    ...
    it('should fetch from database and cache when not in cache', async () => {
      ...
      expect(mockRedisUtils.setCache).toHaveBeenCalledWith(`mold:${moldId}`, mockMoldWithId, 300);
    });
  ...
  describe('retrieveMoldByName', () => {
    it('should return cached mold if available', async () => {
      ...
      expect(result).toEqual(mockMoldWithId);
      expect(mockRedisUtils.getCache).toHaveBeenCalledWith(`mold:name:${moldName}`);
    });
    ...
    it('should fetch from database when not cached', async () => {
      ...
      const mockQuerySnapshot = {
        docs: [{ id: 'test-id', data: jest.fn().mockReturnValue(mockMoldWithId) }],
      };
      mockMoldRepository.findMoldByName.mockResolvedValue(mockQuerySnapshot as any);
      mockFirestoreLib.queryToJson.mockReturnValue([mockMoldWithId] as any);

      const result = await moldService.retrieveMoldByName(moldName);

      expect(result).toEqual(mockMoldWithId);
      expect(mockRedisUtils.setCache).toHaveBeenCalledWith(`mold:name:${moldName}`, mockMoldWithId, 300);
    });
  ...
  describe('updateMoldInFirestore', () => {
    it('should successfully update a mold', async () => {
      ...
      jest.spyOn(moldService, 'retrieveMoldById').mockResolvedValue(updatedMold as any);

      const result = await moldService.updateMoldInFirestore(moldId, updateData);

      expect(result).toEqual(updatedMold);
      expect(mockMoldRepository.updateMold).toHaveBeenCalledWith(moldId, updateData);
  ...
  describe('softRemoveMold', () => {
    it('should successfully soft delete a mold', async () => {
      ...
      const mockWriteResult = { writeTime: Timestamp.now() };
      mockMoldRepository.softDeleteMold.mockResolvedValue(mockWriteResult as any);
