...
describe("verification middleware (unit)", () => {
  ...
  describe("handle role-based authorization for user roles using token", () => {
    const res = { status: jest.fn().mockReturnThis(), json: jest.fn() };
    const next = jest.fn();
  ...
  describe("handle role-based authorization for user roles using cookie", () => {
    const res = { status: jest.fn().mockReturnThis(), json: jest.fn() };
    const next = jest.fn();
