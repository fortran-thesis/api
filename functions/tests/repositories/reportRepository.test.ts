...
describe('reportRepository (integration)', () => {
  ...
  describe('paginate reports', () => {
    it('should successfully paginate', async () => {
      // seed extra reports
      const extraReports = Array.from({ length: 7 }, (_, i) => ({reporter_id: `test_reporter_${i}`,  title: `paginated_report_${i}`, description: 'desc', created_at: Date.now() }));
      ...
      const second = await repo.findAllReports(3, first!.nextPageToken || undefined);
      expect(second && second.snapshot.size).toBe(3);
      const third = await repo.findAllReports(3, second!.nextPageToken || undefined);
