...
describe("userRepository (integration)", () => {
  ...
  it(
    "should not allow duplicate user creation and returns null",
    async () => {
  ...
  it(
    "should not find a non-existent user",
    async () => {
  ...
  it(
    "should update the test user",
    async () => {
      await userRepo.updateFirestoreUser("testuid", {
  ...
  it(
    "should return null when trying to update a non-existent user",
    async () => {
      const updated = await userRepo.updateFirestoreUser("does_not_exist", {
        username: "foo",
      });
  ...
  it(
    "should soft delete the test user",
    async () => {
  ...
  it(
    "should not delete a non-existing user",
    async () => {
      await expect(
        userRepo.deleteFirestoreUser("does_not_exist")
      ).resolves.toBeNull();
    },
    timeout
  );
  ...
  describe("paginate users", () => {
    it(
      ...
      async () => {
        ...
        const second = await userRepo.findAllUsers(
          3,
          first!.nextPageToken || undefined
        );
        expect(second && second.snapshot.size).toBe(3);

        const third = await userRepo.findAllUsers(
          3,
