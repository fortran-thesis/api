...
describe('moldRepository (integration)', () => {
  ...
  describe('paginate molds', () => {
    it('should successfully paginate', async () => {
      // seed extra molds
      const extraMolds = Array.from({ length: 7 }, (_, i) => ({ name: `paginated_mold_${i}`, created_at: Date.now() }));
      ...
      const second = await repo.findAllMolds(3, first!.nextPageToken || undefined);
      expect(second && second.snapshot.size).toBe(3);
      const third = await repo.findAllMolds(3, second!.nextPageToken || undefined);
