...
describe('moldipediaRepository (integration)', () => {
  ...
  it('should add the test moldipedia entry', async () => {
    const testEntry = { title: 'test moldipedia', body: 'body', author_id: 'author', cover_photo: '', tags: [], created_at: Date.now() };
  ...
  describe('paginate moldipedia entries', () => {
    it('should successfully paginate', async () => {
      // seed extra entries
      const extraEntries = Array.from({ length: 7 }, (_, i) => ({author_id: `author_${i}`, title: `paginated_moldipedia_${i}`, body: 'body', cover_photo: '', tags: [], created_at: Date.now() }));
      ...
      const second = await repo.findAllMoldipedia(3, first!.nextPageToken || undefined);
      expect(second && second.snapshot.size).toBe(3);
      const third = await repo.findAllMoldipedia(3, second!.nextPageToken || undefined);
