...
describe("systemRequestRepository (integration)", () => {
  ...
  describe("paginate system requests", () => {
    it("should successfully paginate", async () => {
      // seed extra requests
      const extraRequests = Array.from({length: 7}, (_, i) => ({
        userId: `user_${i}`,
        type: `paginated_type_${i}`,
        payload: {},
        created_at: Date.now(),
      }));
      ...
      const second = await repo.findAllSystemRequests(
        3,
        first!.nextPageToken || undefined
      );
      expect(second && second.snapshot.size).toBe(3);
      const third = await repo.findAllSystemRequests(
        3,
